...
3. [Databases — NoSQL vs SQL](#3-databases--nosql-vs-sql)
...
11. [Zod — Schema Validation](#11-zod--schema-validation)
...
JavaScript was originally a browser language. Node.js brought it to the server. Now the same language runs everywhere — frontend, backend, scripts, CLI tools. This means:
...
let y = 2;   // block-scoped, not hoisted — prefer this
const z = 3; // block-scoped, cannot reassign — use for constants
```
...
```js
console.log(greet()); // Works! — function declarations are hoisted
console.log(sayHi);   // undefined — var is hoisted but not initialized
console.log(sayBye);  // ReferenceError — let/const not hoisted
...
// In function parameters — very common in Express
...
JavaScript is **single-threaded** — it runs one piece of code at a time. But it can handle thousands of concurrent requests because of the **event loop**.
...
```
  ...
  └───────────┬──────────────┘
              │
  ┌───────────▼──────────────┐
  │       Web APIs / libuv   │  ← Handles I/O, timers (in parallel, off-thread)
  └───────────┬──────────────┘
              │
  ┌───────────▼──────────────┐
  │      Callback Queue       │  ← Waits here when ready
  └───────────┬──────────────┘
              │
  ┌───────────▼──────────────┐
  │      Microtask Queue      │  ← Promises go here (higher priority)
  └───────────┬──────────────┘
              │
  ┌───────────▼──────────────┐
  │        Event Loop         │  ← Moves callbacks to call stack when stack is empty
  └──────────────────────────┘
```

**Execution Order:**
1. Synchronous code runs first
2. Microtasks (Promises) run next — before anything else
...
**Real-World Impact:** This is why Node.js can serve 10,000 concurrent requests with a single thread — while waiting for a DB query to return, the event loop handles other requests.
...
```js
// Run multiple promises in parallel — fails fast if any one fails
...
// GOOD — parallel, takes max(time) instead of sum(time)
...
```js
// ❌ MISTAKE 1: Forgetting await — function returns a Promise, not the value
...
// ❌ MISTAKE 2: Not handling errors in async route handlers
app.get('/users', async (req, res) => {
  const users = await db.getUsers(); // If this throws, Express crashes!
  res.json(users);
});

// ✅ FIX: Always wrap in try/catch or use an async wrapper
...
// ❌ MISTAKE 3: Sequential awaits when parallel is possible
...
// ❌ MISTAKE 4: Using await inside forEach — doesn't work as expected!
...
// Output: nextTick → Promise → setTimeout (or setImmediate, order varies) → setImmediate
```

**process.nextTick** runs before any I/O, before Promises even. Use sparingly — abusing it can starve the I/O queue.
...
// Synchronous — BLOCKS the event loop! Only ok for startup scripts
...
// ✅ GOOD — streams file to client, uses constant memory
...
**Best Practice — config.js:**

```js
// config.js — centralize all env vars, validate early
const config = {
...
Data stored in flexible formats — documents, key-value pairs, graphs, etc.
...
**A — Atomicity:** All operations in a transaction succeed or all fail. No partial updates.
...
**C — Consistency:** Data always moves from one valid state to another. Constraints are enforced.
...
**Real-World Decision:**
- **Banking app:** Choose CP — you'd rather be down than show wrong balance
- **Social media feed:** Choose AP — it's ok to see slightly stale data, better to be available
- **E-commerce cart:** Choose AP — let the cart work, reconcile inventory on checkout
...
Database → Collection → Document
  myapp  →   users   → { _id: ..., name: "Alice", email: "alice@..." }
```

MongoDB stores documents as **BSON** (Binary JSON) — supports more types than regular JSON (Date, ObjectId, Binary, etc.)
...
```js
// Embedded — user profile always comes with user
{
  _id: ObjectId("..."),
  name: "Alice",
  email: "alice@example.com",
  ...
  addresses: [            // embedded array (bounded — users have few addresses)
...
-- JSON in PostgreSQL — best of both worlds
CREATE TABLE products (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name VARCHAR(255),
...
calculateDiscount("100", 20); // "100" - 20 = "10020" — string concatenation!

// TypeScript — caught immediately
function calculateDiscount(price: number, discountPercent: number): number {
  return price - (price * discountPercent / 100);
}
calculateDiscount("100", 20); // ❌ Error: Argument of type 'string' not assignable to 'number'
```
...
// never — values that never exist (exhaustive checks, infinite loops)
...
// Partial<T> — makes all fields optional (for PATCH requests)
...
// Pick<T, K> — select only specific fields
...
```json
// tsconfig.json — recommended for Node.js backend
{
  "compilerOptions": {
...
```
  ...
  example.com/users  →  IP (via DNS)       →  request to server
                         93.184.216.34

  Server processes       Server sends           Browser renders
  the request        ←  HTTP response      ←   (HTML, JSON, etc.)
```
...
This lookup is cached at multiple levels (browser, OS, ISP) — that's why DNS changes take time to propagate.

---

## 6.2 HTTP — The Protocol of the Web
...
Response Headers:
  ...
  X-Rate-Limit-Remaining: 95           — Custom header
  CORS headers:
...
Nested resources:
  GET    /users/:id/orders     — orders for a specific user
  POST   /users/:id/orders     — create order for a user
  GET    /users/:id/orders/:orderId — specific order for a user

Actions (when REST doesn't fit):
  POST /users/:id/activate    — activate a user (not a resource)
...
WS:    Client ⟺ Server (persistent, bidirectional, low latency)
```
...
app.get('/poll', async (req, res) => {
  ...
  res.json({ events: [] }); // Timeout — client will poll again
});
...
| Long Polling | Server→Client | Medium | Medium | Simple notifications |
...
gRPC uses **Protocol Buffers** (protobuf) — a binary serialization format that's faster and smaller than JSON.
...
// Router — organize routes by feature
...
// GET /api/v1/users/123 → getUser
// GET /api/v1/users/123/orders → getUserOrders
```
...
const users = await prisma.user.findMany({
  ...
  select: {        // only return specific fields
    ...
    email: true,
    createdAt: true
    // password omitted — never return password!
  }
});
...
│   └── db/           ← Prisma client (shared between apps)
...
```json
// apps/api/package.json — reference the shared package
{
  "dependencies": {
    "@myapp/types": "*"
  }
...
```ts
// apps/web/src/api/users.ts — same types on frontend!
...
Serverless (FaaS — Function as a Service) means you write individual functions instead of managing a server. The cloud provider handles scaling, availability, and infrastructure.

```
Traditional Server:
  Server always running → Always paying → You manage scaling

Serverless:
  Function sleeps → Request arrives → Function wakes (cold start) → Executes → Sleeps again
  Pay per execution, scales automatically to millions
```

**Use cases:**
- Event handlers (image uploaded → resize)
- Cron jobs
- Webhooks
- Low-traffic APIs
...
```js
// Cloudflare Worker — tiny, fast, runs at edge
...
```ts
// Using tsoa — generates OpenAPI from TypeScript decorators
...
Manually writing API fetch functions is tedious and error-prone. When the API changes, you update the OpenAPI spec and regenerate the client — no manual sync needed.
...
Generated code:
```ts
// src/api/users.ts (autogenerated — don't edit!)
...
**Session-based auth:**
```
1. User logs in → Server creates session, stores in DB
...
2. **Horizontal scaling** — more machines (with load balancer)
3. **Cluster module** — use all CPU cores on one machine
...
if (isMainThread) {
  ...
  // Worker thread — runs in separate thread, won't block event loop
...
├── tsconfig.json
├── README.md
└── .npmignore          ← What to exclude from published package
```
...
# For scoped packages — set registry access to public
